# Aether-wall
This is a custom interface for a projector that allows me to interface with the lights and is a utility screen for weather, dates, todo lists and other events.      
## Key Features    
Person detection that triggers wifi lights     
Custom ui designed for a projector on a wall      
Headless feature to run on its own      
Custom interface ui to control todo and light colors       
Custom Lamp for wifi bulbs      

### UI Features      
Displays accurate temperature in local area      
Persisting todo list for morning, afternoon, and evening      
Current time and date along with an analog clock     
Displays upcoming holidays      
Displays a random odd quote from kanye, ron swanson, or an advice list     
Weather Icons for most weather situations      
Icons used in weather by icons8.com      
## UI Implementation     
<img width="4032" height="3024" alt="IMG_2563" src="https://github.com/user-attachments/assets/7e0b861f-0fb4-4050-a092-19a52110dc6b" />     
<img width="1281" height="721" alt="Screenshot 2026-08-09 154123" src="https://github.com/user-attachments/assets/2b00cbf8-b804-4819-b440-802ed3666194" />     
## Interface      
<img width="1280" height="725" alt="Screenshot 2026-08-11 144230" src="https://github.com/user-attachments/assets/2f96d1a1-f125-49b1-beb9-ef055098288e" />     
## Lamp Model           
<img width="514" height="643" alt="Screenshot 2026-06-18 173255" src="https://github.com/user-attachments/assets/e20dcbf9-4fc5-4c4a-bc16-69fec1f6db35" />                 
## Lamp Implementation            
<img width="3024" height="4032" alt="IMG_2565" src="https://github.com/user-attachments/assets/408e32e1-0c9b-46c0-b4f1-8a41e67de7b1" />           
